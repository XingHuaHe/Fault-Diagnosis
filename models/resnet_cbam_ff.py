# 
# Program:
#   基础 ResNet 结构 结合 空间、通道注意力机制 CBAM， 以及在模型的末端，进行简单的特征融合（不同网络深度特在共3个）.
# 
# Data:
#   2021.4.7
# 
...
__all__ = ['resnet18_cbam_ff', 'resnet34_cbam_ff', 'resnet50_cbam_ff', 'resnet101_cbam_ff', 'resnet152_cbam_ff']
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: List[int], stride: int = 1):
        downsample = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Features.
        feature1 = self.layer1(x) # 64*64*64
        feature2 = self.layer2(feature1) # 512*32*32
        feature3 = self.layer3(feature2) # 256*16*16
        feature4 = self.layer4(feature3) # 512*8*8

        # Features reconstruct.
        feature3 = self.upsample3(feature3) # 256*32*32
        feature3 = self.conv2(feature3) # 128*32*32
        feature4 = self.upsample4(feature4) # 512*32*32
        feature4 = self.conv3(feature4) # 128*32*32
...
def resnet18_cbam_ff(pretrained: str = '', **kwargs):
...
def resnet34_cbam_ff(pretrained: str = '', **kwargs):
...
def resnet50_cbam_ff(pretrained: str = '', **kwargs):
...
def resnet101_cbam_ff(pretrained: str = '', **kwargs):
...
def resnet152_cbam_ff(pretrained: str = '', **kwargs):
